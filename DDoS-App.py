...
def ddos_mitc_detection():
        ...
        if IP in pkt:
            ...
            if dst_ip == target_ip and TCP in pkt:
                ...
                if flags & 2 or flags & 4:
                    ...
                    df.to_csv('ddos_and_mitc_attacks.csv', mode='a', index=False, header=not os.path.exists('ddos_and_mitc_attacks.csv'))
    ...
    while time.time() - start_time < duration:
            sniff(prn=detect_attacks, timeout=10)

    # Load the data from the CSV file
    df = pd.read_csv('ddos_and_mitc_attacks.csv')
    ...
    percent_malicious_packets = sum(predictions == 'malicious') / len(predictions) * 100

    percent_malicious = "{:.2f}".format(percent_malicious_packets)
